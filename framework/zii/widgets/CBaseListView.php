...
abstract class CBaseListView extends CWidget
{
	...
	{
		$emptyText=$this->emptyText===null ? Yii::t('zii','Không tìm thấy kết quả.') : $this->emptyText;
	...
	public function renderSummary()
	{
		...
		if($this->enablePagination)
		{
			...
			if(($summaryText=$this->summaryText)===null)
				$summaryText=Yii::t('zii','Có {count} kết quả. Hiển thị {start}-{end} của {count} kết quả.',$total);
		...
		{
			if(($summaryText=$this->summaryText)===null)
				$summaryText=Yii::t('zii','Có {count} kết quả.',$count);
			echo strtr($summaryText,array(
				'{count}'=>$count,
				'{start}'=>1,
				'{end}'=>$count,
